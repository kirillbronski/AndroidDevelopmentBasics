...
typealias ValueListener<T> = (ResponseResult<T>) -> Unit
...
class LazyListenersSubject<A : Any, T : Any>(
    ...
    private fun publishIfNotCancelled(record: FutureRecord<T>, argument: A, result: ResponseResult<T>) {
        if (record.cancelled) return
        publish(argument, result)
    }

    private fun publish(argument: A, result: ResponseResult<T>) = handlerExecutor.execute {
    ...
    private class FutureRecord<T>(
        var future: Future<*>?,
        var lastValue: ResponseResult<T> = Empty(),
