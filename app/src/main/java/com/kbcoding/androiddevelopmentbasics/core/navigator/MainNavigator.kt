...
class MainNavigator(
    ...
    override fun toast(messagesRes: Int) {
        Toast.makeText(getApplication(), messagesRes, Toast.LENGTH_SHORT).show()
    }

    override fun getString(messagesRes: Int): String {
        return getApplication<Application>().getString(messagesRes)
    }

    fun launchFragment(activity: MainActivity, screen: BaseScreen, addToBackStack: Boolean = true) {
        val fragment =
            screen.javaClass.enclosingClass.getDeclaredConstructor().newInstance() as Fragment
